...
export async function Counter_PK_Module(context: any) {
    ...
    if (id == null) { return }
    if (context.text == `!пкметр`) { users_pk[id].mode = true; await context.send(`✅ Активирован режим замера ПК. Вводите рп посты, или любой другой текст длины, какая нужна вам! Когда закончите, нажмите !обнулить`); return true }
	if (context.text == `!обнулить`) { users_pk[id].mode = false; users_pk[id].text = ``; await context.send(`⛔ Обнулен и выключен режим замера ПК`); return true }
