...
async function Alliance_Facult_Return(context: any, data: any, alliance: Alliance, user: User) {
    ...
    await context.send(`${ico_list['cancel'].ico} Отмена меню управления факультетами ролевого проекта ${alliance.id}-${alliance.name}`, { keyboard: Keyboard.builder().callbackButton({ label: '🌐 В ролевую', payload: { command: 'alliance_enter' }, color: 'primary' }).inline() })
    return res
}
